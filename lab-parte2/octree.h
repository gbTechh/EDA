...
private:
  ...
  float distanceToNode(const Point &target);

  void exportOctreeCubesOBJ(Octree *nodo, std::ofstream &out,
                            int &vertexOffset);

  void exportOctree(std::ofstream &file, int &vertexCount);
  void exportCube(std::ofstream &file, int &vertexCount);
...
public:
  ...
  Point find_closest(const Point &, float radius);
  void printTree(int depth = 0);

  void findNodeForPoint(const Point &p, Point &nodeBottomLeft, float &nodeH);
