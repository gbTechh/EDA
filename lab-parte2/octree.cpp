...
void Octree::exportCube(std::ofstream &file, int &vertexCount) {
  float x = bottomLeft.x;
  float y = bottomLeft.y;
  float z = bottomLeft.z;

  float s = h; // tamaño del cubo

  // 1. Vértices
  std::vector<Point> vertices = {{x, y, z},
                                 {x + s, y, z},
                                 {x + s, y + s, z},
                                 {x, y + s, z},
                                 {x, y, z + s},
  ...
  // 2. Caras
  std::vector<std::array<int, 4>> caras = {{0, 1, 2, 3}, {4, 5, 6, 7},
                                           {0, 1, 5, 4}, {2, 3, 7, 6},
                                           {1, 2, 6, 5}, {0, 3, 7, 4}};

  int base = vertexCount;
  for (auto &f : caras) {
    file << "f " << base + f[0] + 1 << " " << base + f[1] + 1 << " "
         << base + f[2] + 1 << " " << base + f[3] + 1 << "\n";
  }

  vertexCount += 8;
}
void Octree::exportOctree(std::ofstream &file, int &vertexCount) {
  ...
  // Exportar solo hojas con puntos
  if (esHoja && !points.empty()) {
    exportCube(file, vertexCount);
  }

  // Recorrer hijos
  for (int i = 0; i < 8; i++) {
    if (children[i] != nullptr) {
      children[i]->exportOctree(file, vertexCount);
    }
  }
}
...
void Octree::exportOBJ(const std::string &filename) {
  ...
  int vertexCount = 0;
  this->exportOctree(file, vertexCount);

  file.close();
  std::cout << "Archivo OBJ generado: " << filename << std::endl;
}