...
RTree::RTree(const RTree &other) : RTree() { CopyRec(m_root, other.m_root); }

RTree::~RTree() { Reset(); }

// Initialize Branch with dataId
void RTree::Insert(const int a_min[2], const int a_max[2],
                   vector<pair<int, int>> &a_dataId) {
...
bool RTree::InsertRectRec(const Branch &a_branch, Node *a_node,
  ...
  if (a_node->m_level > a_level) {
    ...
    int index =
        PickBranch(&a_branch.m_rect, a_node); // ChooseLeaf->Choose subtree

    bool childWasSplit =
        InsertRectRec(a_branch, a_node->m_branch[index].m_child, &otherNode,
                      a_level); // ChooseLeaf->Descend until
...
void RTree::Remove(const int a_min[2], const int a_max[2],
                   const vector<pair<int, int>> &a_dataId) {
...
void RTree::CopyRec(Node *current, Node *other) {
  ...
  if (current->IsInternalNode()) {
    for (int index = 0; index < current->m_count; ++index) {
      ...
      std::copy(otherBranch->m_rect.m_min, otherBranch->m_rect.m_min + 2,
                currentBranch->m_rect.m_min);

      std::copy(otherBranch->m_rect.m_max, otherBranch->m_rect.m_max + 2,
...
void RTree::Reset() { RemoveAllRec(m_root); }
...
void RTree::FreeNode(Node *a_node) { delete a_node; }

ListNode *RTree::AllocListNode() { return new ListNode; }

void RTree::FreeListNode(ListNode *a_listNode) { delete a_listNode; }
...
int RTree::PickBranch(const Rect *a_rect, Node *a_node) {
  ...
  for (int index = 0; index < a_node->m_count; ++index) {
    ...
    if ((increase < bestIncr) || firstTime) {
      ...
      bestIncr = increase;
      firstTime = false;
    } else if ((increase == bestIncr) && (area < bestArea)) { // on tie
      best = index;
      bestArea = area;
      bestIncr = increase;
    }
...
Rect RTree::CombineRect(const Rect *a_rectA,
                        const Rect *a_rectB) // Calcula los rangos en que esta
{
...
float RTree::CalcRectArea(Rect *a_rect) {
  ...
  area = ((float)a_rect->m_max[0] - (float)a_rect->m_min[0]) *
         ((float)a_rect->m_max[1] - (float)a_rect->m_min[1]);

  return area;
}

void RTree::GetBranches(Node *a_node, const Branch *a_branch,
...
void RTree::ChoosePartition(PartitionVars *a_parVars, int a_minFill) {
  ...
  while (
    ...
    biggestDiff = (float)-1;
    for (int index = 0; index < a_parVars->m_total; ++index) { // PickNext
...
void RTree::InitParVars(PartitionVars *a_parVars, int a_maxRects,
                        int a_minFill) {
...
bool RTree::RemoveRect(Rect *a_rect, const vector<pair<int, int>> &a_id,
                       Node **a_root) {
...
bool RTree::RemoveRectRec(Rect *a_rect, const vector<pair<int, int>> &a_id,
                          Node *a_node, ListNode **a_listNode) {

  if (a_node->IsInternalNode()) {
    for (int index = 0; index < a_node->m_count; ++index) {
      if (Overlap(a_rect, &(a_node->m_branch[index].m_rect))) {
        if (!RemoveRectRec(a_rect, a_id, a_node->m_branch[index].m_child,
                           a_listNode)) {
...
void RTree::Search(Node *a_node, Rect *a_rect,
                   vector<vector<pair<int, int>>> &results) {
  // Caso 1: si el nodo es hoja → revisamos datos
  if (a_node->IsLeaf()) {
    for (int i = 0; i < a_node->m_count; i++) {
      Branch &br = a_node->m_branch[i];
      if (Overlap(&br.m_rect, a_rect)) {
        // Coincidencia: guardamos los datos asociados
        results.push_back(br.m_data);
      }
    }
  }
  // Caso 2: si es un nodo interno → descender recursivamente
  else {
    for (int i = 0; i < a_node->m_count; i++) {
      Branch &br = a_node->m_branch[i];
      if (Overlap(&br.m_rect, a_rect)) {
        // Si el MBR del hijo intersecta con la query → bajar
        Search(br.m_child, a_rect, results);
      }
    }
  }
}
