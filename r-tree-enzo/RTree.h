...
public:
  ...
  void Insert(const int a_min[2], const int a_max[2],
              vector<pair<int, int>> &a_dataId);
  void Remove(const int a_min[2], const int a_max[2],
  ...
  void Search(const Rect &S, Node *T, vector<vector<pair<int, int>>> &results);
  /// get the MBRs;
...
protected:
  ...
  bool RemoveRect(Rect *a_rect, const vector<pair<int, int>> &a_id,
                  Node **a_root);
  bool RemoveRectRec(Rect *a_rect, const vector<pair<int, int>> &a_id,
  ...
  float m_unitSphereVolume; ///< Unit sphere constant for required number of
                            ///< dimensions

  Rect Search(Rect Ei, Node *Ep);
};
