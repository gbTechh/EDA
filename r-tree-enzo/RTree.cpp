...
void RTree::Search(const Rect& S, Node* T, vector<vector<pair<int,int>>>& results)
{
    if (!(T->IsLeaf())) 
	{
		for (int i = 0; i < T->m_count; i++) 
		{
            Branch E = T->m_branch[i];
            if (Overlap(&E.m_rect, const_cast<Rect*>(&S))) 
			{
                Search(S, E.m_child, results);
            }
        }
    } 
	else 
