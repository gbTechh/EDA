...
int main(int argc, char *argv[])
{
 ...
 //Rect rect = rtree.MBR(vpoints[0]);
  ...
  cout << "\n--- SEARCH TESTS ---\n";
  // Para que funcione la funcion, el atributo m_root ahora es publico
  // Si deberian devolver algo
  Rect query1(10, 10, 60, 60); 
  cout << "Searching in rectangle: [(" << query1.m_min[0] << "," << query1.m_min[1] 
       << "),(" << query1.m_max[0] << "," << query1.m_max[1] << ")]\n";

  vector<vector<pair<int,int>>> results1;
  rtree.Search(query1, rtree.m_root, results1);

  cout << "Found " << results1.size() << " objects:\n";
  for (auto &obj : results1) 
  {
      print_pair(obj);
      cout << endl;
  }
  ...
  Rect query2(200, 200, 250, 250);
  cout << "\nSearching in rectangle: [(" << query2.m_min[0] << "," << query2.m_min[1] 
       << "),(" << query2.m_max[0] << "," << query2.m_max[1] << ")]\n";

  vector<vector<pair<int,int>>> results2;
  rtree.Search(query2, rtree.m_root, results2);

  cout << "Found " << results2.size() << " objects (should be 0).\n";

  // Area pequeña
  Rect query3(50, 55, 55, 65);
  cout << "\nSearching in rectangle: [(" << query3.m_min[0] << "," << query3.m_min[1] 
       << "),(" << query3.m_max[0] << "," << query3.m_max[1] << ")]\n";

  vector<vector<pair<int,int>>> results3;
  rtree.Search(query3, rtree.m_root, results3);

  cout << "Found " << results3.size() << " objects:\n";
  for (auto &obj : results3) {
      print_pair(obj);
      cout << endl;
  }
