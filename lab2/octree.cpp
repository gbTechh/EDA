...
void Octree::findClosestHelper(const Point &target, float radius,
                               ...
                               bool &found) {
  // Verificar si este nodo puede contener puntos dentro del radio
  float node_distance = 0.0f;

  // Calcular distancia mínima desde el punto objetivo al cubo del nodo
  float dx = std::max(
      0.0f, std::max(bottomLeft.x - target.x, target.x - (bottomLeft.x + h)));
  float dy = std::max(
      0.0f, std::max(bottomLeft.y - target.y, target.y - (bottomLeft.y + h)));
  float dz = std::max(
      0.0f, std::max(bottomLeft.z - target.z, target.z - (bottomLeft.z + h)));
  node_distance = std::sqrt(dx * dx + dy * dy + dz * dz);

  // Si la distancia mínima al nodo es mayor que el radio, no buscar aquí
  if (node_distance > radius) {
    return;
  }

  // Si es un nodo hoja, revisar todos los puntos
  if (children[0] == nullptr) {
    for (const Point &punto : points) {
